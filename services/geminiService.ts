...
import { StoryRequest, StoryBoardResponse, CyberAdviceResponse } from "../types";

// Define cybersecurity schema definition
const vulnerabilitySchema: Schema = {
  type: Type.OBJECT,
  properties: {
    title: { type: Type.STRING },
    risk: { type: Type.STRING, description: "Risk level: منخفض, متوسط, مرتفع, or حرِج" },
    description: { type: Type.STRING },
    fix: { type: Type.STRING }
  },
  required: ["title", "risk", "description", "fix"]
};
...
const commandSchema: Schema = {
  ...
  properties: {
    ...
    description: { type: Type.STRING },
    promptText: { type: Type.STRING, description: "English prompt engineer command for LLM to execute or understand the security response of this tool." }
  },
  required: ["tool", "command", "description", "promptText"]
};

const cyberAdviceSchema: Schema = {
  type: Type.OBJECT,
  properties: {
    title: { type: Type.STRING },
  ...
  required: ["title", "riskLevel", "summary", "vulnerabilities", "commands", "recommendations"]
};
...
export const generateCyberAdvice = async (systemDescription: string, focusArea: string): Promise<CyberAdviceResponse> => {
  const apiKey = process.env.API_KEY;
  if (!apiKey) {
    throw new Error("API Key is missing. Please check your environment variables.");
  }
  ...
  const prompt = `
    You are an elite hands-on white-hat hacker, chief penetration tester, and cybersecurity specialist.
    Your task is to analyze the following system or scenario, identify top vulnerabilities, and provide specialized testing / diagnostic commands and prompt templates.

    - System or target description: ${systemDescription}
    - Specific focus area or critical asset: ${focusArea}

    GUIDELINES:
    1. Overall report MUST be detailed and highly technical.
    2. Vulnerabilities, fixes, summary, title, and general recommendations MUST be returned in ARABIC.
    3. Under 'commands', provide actual tools used in ethical hacking/penetration testing (like Nmap, Gobuster, SQLMap, Metasploit, Hydra or customized python scripts).
       The specific 'command' terminal code MUST be in standard technical code (English/Linux shell syntax).
       The 'promptText' inside commands must contain a powerful, fully-formed English prompt template that a security engineer can input into an AI / LLM (such as Gemini or ChatGPT) to help analyze the scan outcome, inspect code snippets for this issue, or engineer a cyber defense script.
    4. Provide the result strictly in JSON matching the defined schema.
  `;
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        responseMimeType: "application/json",
        responseSchema: cyberAdviceSchema,
        thinkingConfig: { thinkingBudget: 4096 }
      }
    });

    if (response.text) {
      return JSON.parse(response.text) as CyberAdviceResponse;
    } else {
      throw new Error("لم يتم إنشاء أي محتوى من Gemini لهذا التحليل.");
    }
  } catch (error) {
    console.error("Gemini Cyber Advice Error:", error);
    throw error;
  }
};